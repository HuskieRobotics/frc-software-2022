...
public class LimelightMath extends SubsystemBase {
    ...
    public double getIdealVelocity() {
        ...
        resetVXVY();
        v = Math.sqrt((vx * vx) + (vy * vy)) * (LimelightConstants.Velocity_Multiplier)
                / (2 * Math.PI * (LimelightConstants.Flywheel_Radius_IN)) * (LimelightConstants.Ticks_Per_One_Rotation)
                / 10;
