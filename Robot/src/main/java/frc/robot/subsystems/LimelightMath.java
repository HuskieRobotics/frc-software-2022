...
public class LimelightMath extends SubsystemBase {
  ...
  public LimelightMath() {
    ...
    this.shooterAngleNT = Shuffleboard.getTab("LimeTuning").add("shooterAngle", 0.0).getEntry();
    this.limelightDistanceInNT =
        Shuffleboard.getTab("LimeTuning").add("DistanceOutput", 0.0).getEntry();
    this.velocityNT = Shuffleboard.getTab("LimeTuning").add("IdealVelocity", 0.0).getEntry();
    this.tyNT = Shuffleboard.getTab("LimeTuning").add("ty", 0.0).getEntry();

    this.getvx = Shuffleboard.getTab("LimeTuning").add("vx", 0.0).getEntry();

    this.getvy = Shuffleboard.getTab("LimeTuning").add("vy", 0.0).getEntry();
  }
  ...
  private void resetVXVY() {
    ...
    vx =
        Math.sqrt(-(d1 * d1) * d2 * g + d1 * (d2 * d2) * g)
            / (Math.sqrt(2) * Math.sqrt(-d2 * h1 + d1 * h2));
    vy =
        (g * ((d2 * d2 * h1) - (d1 * d1 * h2)))
            / (Math.sqrt(d1 * d2 * (-d1 + d2) * g) * Math.sqrt(-2 * d2 * h1 + 2 * d1 * h2));
  }
  ...
  public double getIdealVelocity() {
    ...
    v =
        Math.sqrt((vx * vx) + (vy * vy))
            * (LimelightConstants.Velocity_Multiplier)
            / (2 * Math.PI * (LimelightConstants.Flywheel_Radius_IN))
            * (LimelightConstants.Ticks_Per_One_Rotation)
            / 10;
