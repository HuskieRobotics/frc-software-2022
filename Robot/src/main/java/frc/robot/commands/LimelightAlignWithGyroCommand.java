...
/**
 * This command, when executed, instructs the drivetrain subsystem to rotate to aim at the hub.
 * This command initially uses the Limelight to determine the offset and then repeatedly uses the
 * gyro to eliminate that initial offset. The advantage of this approach compared to using the
 * Limelight repeatedly is that the Limelight has more noise than the gyro.
 * 
...
public class LimelightAlignWithGyroCommand extends CommandBase {
     ...
     * for the drivetrain rotation and determines the setpoint (in degrees) of the gyro based on
     * the Limelight and the current gyro value. It is critical that this initialization occurs in
     * this method and not the constructor as this command is constructed once when the
     ...
     * obtains the rotational velocity from the PID controller and instructs the drivetrain
     * subsytem to rotate to aligned to the hub.
     */
    @Override
    public void execute() {
     ...
     * the drivetrain is aimed based on the gyro.
     */
    @Override
