 ...
 * 
 * Requires: the flywheel subsystem (the drivetrain subsystem is not required as it is only queried
 *      to get the distance to the hub)
 * Finished When: the flywheel velocity is at the setpoint
 * At End: leaves the flywheel spinning
 */
public class LimelightSetFlywheelVelocityCommand extends CommandBase {
    ...
    /**
     * This method is invoked once when this command is scheduled. This command assumes that the
     * robot is not in motion as it only queries the distance to the hub when initialized.
     */
    @Override
     ...
     * sets the setpoint of the flywheel velocity to the desired velocity.
     */
    @Override
    public void execute() {
    ...
    /**
     * This method will be invoked when this command finishes or is interrupted. It does not stops
     ...
     * the velocity of the flywheel has reached the specified setpoint.
     */
    @Override
