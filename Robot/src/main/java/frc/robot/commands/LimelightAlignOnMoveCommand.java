 ...
 * 
 * Requires: the drivetrain and flywheel subsystems (the collector subsystem is not a requirement
 *      as it is not controlled and only queried for its current state)
 * Finished When: the drivetrain is aimed, the flywheel is at the specified speed, and the robot is
 *      within optimal shooting distance
...
public class LimelightAlignOnMoveCommand extends CommandBase {
    ...
    // update constructor to use suppliers instead of Joysticks references like the default drive command
    /**
     ...
     * instructs the drivetrain subsytem to move translationally while keeping aligned to the hub,
     * if visible. If the hub is not visible, the joystick input controls the rotation of the drivetrain.
     */
    @Override
    public void execute() {
        ...
        // if we can't see the target, the joystick controls the rotation
            ...
            drivetrainSubsystem.drive(
                    -RobotContainer.modifyAxis(joystick0.getY()) * DrivetrainSubsystem.MAX_VELOCITY_METERS_PER_SECOND, // joystick x
                    -RobotContainer.modifyAxis(joystick0.getX()) * DrivetrainSubsystem.MAX_VELOCITY_METERS_PER_SECOND, // joystick y
